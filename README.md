# xcloud-framework：springcloud + maven多模块项目
...
> 2. xcloud-gateway 路由网管 (springcloud+gateway)
...
> 9. xcloud-dubbo-api dubbo接口（独立模块）

> 10. xcloud-dubbo-consumer dubbo消费者（独立模块）

> 11. xcloud-dubbo-productor dubbo 生产者（独立模块）
...
2.3 使用 tail -f nohup.out 查看控制台日志
```

4. 在linux服务器上面，配置好jdk、maven、git命令后，通过`git clone sb-xxx`拉取工程后，执行`./run.sh start test`命令来执行
...
> 按上面顺序启动项目（弄得有点复杂了，不建议是使用dubbo做微服务框架）


###  oauth2 是参考这位同行构建的 ，地址：https://github.com/liangxiaobo/test-security-oauth2
...
## 1.1创建xcloud-oauth2项目 (用于获取授权token)
  1.1 创建项目依赖 
...
```
## 1.3SecurityConfiguration 继承 WebSecurityConfigurerAdapter
...
>
> ##  写一个端点测试Controller TestEndPointController
``` java
@RestController
